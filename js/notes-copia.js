...
var app = {
	...
	salvarNota: function() {
		if (QueAccion == "M"){
			...
			if ( document.getElementById('qfecha').value === '') {
				//-- borrar elemento i  -> app.model.notas.splice(i, 1);
			};
	...
	anadirNotasALista: function() {
		...
		notasDivs = notasDivs + "<tr><th>Fecha</th><th>Peso</th><th>Tensión<br>Baja</th><th>Tensión<br>Alta</th></tr>";
		
		for (var i in notas) {
			var qfecha = "<td>" + notas[i].qfecha + "</td><td>" + notas[i].qpeso + "</td><td>" + notas[i].qtbaja + "</td><td>" + notas[i].qtalta + "</td>";
	...
	construirNota: function() {
		...
		if ( document.getElementById('qfecha').value != '') {
			notas.push({"qfecha": app.extraerQFecha(), "qpeso": app.extraerQPeso(), "qtbaja": app.extraerQTbaja(), "qtalta": app.extraerQTalta() });
		};
	},
